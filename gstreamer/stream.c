...
int main(int argc, char** argv){
    ...
    data.mpegtsmux_sink = gst_element_factory_make("mpegtsmux", "mpegtsmux_sink");
    data.hlssink = gst_element_factory_make("hlssink", "hlssink");

    // Create the empty pipeline
    data.pipeline = gst_pipeline_new("rtsp-pipeline");

    if(!data.pipeline || !data.source || !data.rtph264depay_sink || !data.h264parse_sink || !data.mpegtsmux_sink) {
        g_printerr("Not all elements could be created.\n");
        return -1;
    }

    // Building the pipeline
    gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.rtph264depay_sink, data.h264parse_sink, data.mpegtsmux_sink, data.hlssink, NULL);
    if(gst_element_link_many(data.rtph264depay_sink, data.h264parse_sink, data.mpegtsmux_sink, data.hlssink,  NULL) != TRUE){
